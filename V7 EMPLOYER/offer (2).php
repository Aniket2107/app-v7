...
if($adminrights){
$sql = "SELECT a.jobtitle as jobtitle,a.jobid,b.fname as fname,b.email as email,b.notice as notice,d.name as agency,d.registerid,b.contactnumber,b.cv as cv,b.resume,c.status as candidatestatus,c.viewnotes,b.jobtype,b.desiredsalary,b.expectedcurrency,b.typesalary,c.submitdate,e.onlinestatus as onlinestatus,e.lastlogin as lastlogin,c.candidateid as candidateid, c.recruiterid as recruiterid, c.viewed as viewed,c.employernotes,c.recruiternotes,c.viewedtime,s.firstname as subuser FROM submitted_candidates c,jobs a,candidates b,companyprofile d,members e,members s WHERE s.companyid='$cid' and s.memberid=a.memberid and a.jobid=c.jobid and b.id=c.candidateid and c.status='Offered'  and d.id=e.companyid and e.memberid=c.recruiterid  and submitdate >= CURDATE()-INTERVAL 6 MONTH $jobtitleres $candidateres $recruiterres order by submitdate desc";
}else
{
  $sql = "SELECT a.jobtitle as jobtitle,a.jobid,b.fname as fname,b.email as email,b.notice as notice,d.name as agency,d.registerid,b.contactnumber,b.cv as cv,b.resume,c.status as candidatestatus,c.viewnotes,b.jobtype,b.desiredsalary,b.expectedcurrency,b.typesalary,e.onlinestatus as onlinestatus,e.lastlogin as lastlogin,c.submitdate,c.candidateid as candidateid, c.recruiterid as recruiterid, c.viewed as viewed,c.employernotes,c.recruiternotes,c.viewedtime,e.firstname as subuser FROM submitted_candidates c,jobs a,candidates b,companyprofile d,members e WHERE a.memberid='$mid' and a.jobid=c.jobid and b.id=c.candidateid and c.status='Offered'  and d.id=e.companyid and e.memberid=c.recruiterid  and submitdate >= CURDATE()-INTERVAL 6 MONTH $jobtitleres $candidateres $recruiterres order by submitdate desc";
} 
...
$checklastlogin = $row['lastlogin'];
   ...
   <td><input type="checkbox"  value="'.$row['candidateid'].','.$row['jobid'].'" name="allcheck[]"> <b><a class="title" href="#" data-toggle="modal" data-target="#candidate-modal" id='.$row['candidateid'].'>'.$row['fname'].'</a></b>'.$viewed.'<br>';	
  	...
  	echo ' | '.$row['candidateid'].'</small> | '.$row['jobtype'].' <br><small>Employer Notes: <b>'.$row['employernotes'].'</b></small> <br><small>Recruiter Notes: <b>'.$row['recruiternotes'].'</b></small></td>
