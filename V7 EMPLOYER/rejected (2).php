...
if($adminrights){
  $sql = "SELECT a.jobtitle as jobtitle,a.jobid,d.name as agency,d.registerid,b.fname as fname,b.contactnumber,b.cv as cv,b.resume,c.status as candidatestatus,c.prestatus,c.viewnotes,b.jobtype,b.desiredsalary,b.expectedcurrency,b.typesalary,c.submitdate,e.onlinestatus as onlinestatus,e.lastlogin as lastlogin,c.viewed as viewed,c.viewedtime,c.candidateid as candidateid, c.recruiterid as recruiterid,c.comment,f.firstname as subuser FROM submitted_candidates c,jobs a,candidates b,companyprofile d ,members e, members f WHERE a.memberid=f.memberid and f.companyid='$cid' and a.jobid=c.jobid and b.id=c.candidateid and c.status='Rejected' and d.id=e.companyid and e.memberid=c.recruiterid $recruiterres $jobtitleres $candres $jobownerres order by submitdate desc"; 
}  else
{
$sql = "SELECT a.jobtitle as jobtitle,a.jobid,d.name as agency,d.registerid,b.fname as fname,b.contactnumber,b.cv as cv,b.resume,c.status as candidatestatus,c.prestatus,c.viewnotes,b.jobtype,b.desiredsalary,b.expectedcurrency,b.typesalary,c.viewed as viewed,c.viewedtime,c.submitdate,e.onlinestatus as onlinestatus,e.lastlogin as lastlogin,c.candidateid as candidateid, c.recruiterid as recruiterid,c.comment,e.firstname as subuser FROM submitted_candidates c,jobs a,candidates b,companyprofile d ,members e WHERE a.memberid='$mid' and a.jobid=c.jobid and b.id=c.candidateid and c.status='Rejected' and d.id=e.companyid and e.memberid=c.recruiterid $recruiterres $jobtitleres $candres  order by submitdate desc"; 
}
...
$checklastlogin = $row['lastlogin'];
		...
		if($row['prestatus']=='Awaiting Feedback')
			...
			echo'
          <tr>
  <td ><input type="checkbox"  value='.$row['candidateid'].' name="allcheck[]"> <b><a class="title" href="#" data-toggle="modal" data-target="#candidate-modal" id='.$row['candidateid'].'>'.$row['fname'].'</a></b>'.$viewed.'<br>';	
  	 {echo date("j F, Y",strtotime($row['submitdate']));}
  	echo ' | '.$row['candidateid'].'</small> | '.$row['jobtype'].' </td>
