...
function rsubcount($link,$jid){
 $ccount = mysqli_fetch_array(mysqli_query($link,('SELECT COUNT(id) as ccount  FROM request  where jobid = "'.$jid.'"')));
...
$content="You have not posted any jobs yet. Click here to <a href=https://www.recruitinghub.com/employer/submit-a-job><b>Post a Job</b></a> or choose <a href=https://www.recruitinghub.com/employer/postdashboard><b>Recruiter Plans</b></a>";
?>
<!doctype html>
<html>
<head>
...
<div class="account-navi-content">
  ...
  <div class="row">
    ...
    </span> | <span class="d-inline-block" tabindex="0" data-toggle="tooltip" title="Click Agencies Link -> Select All & Click Action -> Message Agency -> Type your Message and send so it can be sent to all engaged agencies">Want to communicate to all agencies at once? <i class="fa fa-question-circle"></i>
    </span> </h3>
  </header>
...
$checkpriority=$row['priority'];
 ...
 <td><b><a href="/employer/all?jobtitle='.urlencode($row['jobtitle']).'&candidate=&agency=&status=&submit=Search" >'.subcount($link,$row['jobid']).' Candidates</a></b></td>';
...
if($adminrights){
 ...
 echo'
     ...
     <td><b><a class="title" href="#" data-toggle="modal" data-target="#view-modal" id='.$row['jobid'].'>'.$row['requests'].' Agencies</b></a></td>
     <td>'.$row['country'].'</td>
