...
elseif(isset($_POST['submit'])){
	...
	$empduplicatenotes=htmlspecialchars($_POST['reason'],ENT_QUOTES);
	
	$replacewith = "hiddentext";
    $empduplicatenotes = preg_replace("/[\w-]+@([\w-]+\.)+[\w-]+/", $replacewith, $empduplicatenotes);
    $empduplicatenotes = preg_replace("/^(\(?\d{3}\)?)?[- .]?(\d{3})[- .]?(\d{4})$/", $replacewith, $empduplicatenotes);
    $empduplicatenotes = preg_replace("/^(\(?\d{4}\)?)?[- .]?(\d{3})[- .]?(\d{4})$/", $replacewith, $empduplicatenotes);
	...
	mysqli_query($link,"Update jobs INNER JOIN submitted_candidates ON submitted_candidates.jobid = jobs.jobid set submitted_candidates.empduplicatenotes='$empduplicatenotes',submitted_candidates.status='Duplicate',submitted_candidates.feedbackdate=now(),submitted_candidates.empduplicatenotesdate=now() WHERE submitted_candidates.candidateid='$candidateid' and jobs.memberid='$jmid' and submitted_candidates.jobid='$jid'");
	...
	$namerow1=mysqli_fetch_assoc($namesql1);
		...
		$subject = "Recruiting Hub - (".$namerow1['fname'].") Marked as Duplicate & Reason Added by ".$fetchempname['name']." for ".$jobtitle."";
		$message = '<html>
		<body bgcolor="#FFFFFF">
			<div>
        ...
        <table border="0" cellpadding="0" cellspacing="0" width="100%">
          <tbody>
            ...
            color: #a02121;">  PLEASE DO NOT REPLY TO THIS EMAIL AS EMAILS ARE NOT MONITORED FOR THIS EMAIL ID. LOGIN TO YOUR ACCOUNT TO RESPOND AS APPROPRIATE </p>   
					  ...
					  <p>Candidate marked as Duplicate & Notes added by <strong>'.$fetchempname['name'].'</strong> for <strong>'.$namerow1['fname'].'</strong> submitted for <strong>'.$jobtitle.'</strong>  </p>
					  <p>REASON: <strong><p style = "font-size: 14px;
            color: #a02121;">'.$empduplicatenotes.'</strong></p>
					  <p>Please open the Profile from your Recruiter login to read the Duplicate Notes. Please check with the candidate again. If you already have taken RTR consent from candidate or if found not duplicate please click Action and Add Duplicate Notes to ask client for further clarification on why the cv has been marked as duplicate. Please be polite to the client while checking about duplicacy.</p>
...
'subject'=>$subject,'message'=>$message,'cc'=>$r['email'],'cc2'=>'noreply@recruitinghub.com','send'=>true];
	...
	$to =$fetchemail2['email'] ;
		$from = "noreply@recruitinghub.com";
		$subject = "Recruiting Hub - (".$namerow1['fname'].") Employer Duplicate Notes Added by ".$fetchempname['name']." for ".$jobtitle."";
		$message = '<html>
		<body bgcolor="#FFFFFF">
			<div>
<table style="border-right:1px solid #d5d8d9;border-left:1px solid #d5d8d9;font-family:Arial,Helvetica,sans-serif;font-size:12px;color:#000" align="center" bgcolor="#792785" border="0" cellpadding="0" cellspacing="0" width="620">
        ...
        <table border="0" cellpadding="0" cellspacing="0" width="100%">
          <tbody>
            ...
            color: #a02121;">  PLEASE DO NOT REPLY TO THIS EMAIL AS EMAILS ARE NOT MONITORED FOR THIS EMAIL ID. LOGIN TO YOUR ACCOUNT TO RESPOND AS APPROPRIATE </p> 
					  ...
					  <p>Employer Duplicate Notes have been added by <strong>'.$fetchempname['name'].'</strong> submitted for job ->  <strong>'.$jobtitle.'</strong>  
					  <br><br>
					  NOTES ADDED: <p style = "font-size: 14px;
            color: #a02121;"><strong>'.$empduplicatenotes.'</strong></p>
					  <p style = "font-size: 14px;
            color: #a02121;">This is an automated email sent by Recruiting Hub to let you know that the Employer has Duplicated your submission. The recruiter that submitted your profile is copied in this email who will liaise with you for further details. </p>  
...
'subject'=>$subject,'message'=>$message,'cc'=>$fetchemail['email'],'cc2'=>'noreply@recruitinghub.com','send'=>true];
	AsyncMail($params);
	 ...
	 echo "<script>
alert('Candidate marked as Duplicate. An email notification has now been sent to the Recruiter & Candidate');
...
?>
<?php  foreach($nameres as $namevalue) { echo '<h4 class="modal-title" id="duplicate-modal-label"><strong>'.$namevalue['fname'].'</strong></h4>'; } ?>
<div id="feedback_name">Add Duplicate Reason - (An Automated Email Notification will be sent to the Recruiter) </div>
                   <?php if(!$errormsg){ 
			  			if($content)
							echo $content;
						else{ ?>
                            <form action="empduplicatenotes" method="post">
