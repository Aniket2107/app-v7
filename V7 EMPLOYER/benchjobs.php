...
if($adminrights){
  $sql = "SELECT j.*,m.firstname as subuser FROM benchjobs j,members m WHERE j.memberid=m.memberid and m.companyid='$companyid' and j.status not in ('Closed','Filled','Hold') and j.jobtitle like '%$jobtitle%' order by j.updatedate desc"; 


}else{
  $sql = "SELECT * FROM benchjobs WHERE memberid='$mid' and status not in ('Closed','Filled','Hold') $jobtitleres order by updatedate desc"; 
}
}else{
if($adminrights){

  $sql = "SELECT j.*,m.firstname as subuser FROM benchjobs j,members m WHERE j.memberid=m.memberid and m.companyid='$companyid' and j.status not in ('Closed','Filled','Hold') order by updatedate desc"; 

}else{
  $sql = "SELECT * FROM benchjobs WHERE memberid='$mid' and status not in ('Closed','Filled','Hold') order by updatedate desc"; 
}

}

function subcount($link,$jid){
 $ccount = mysqli_fetch_array(mysqli_query($link,('SELECT COUNT(id) as ccount  FROM submitted_benchcandidates  where jobid = "'.$jid.'"')));
...
function rsubcount($link,$jid){
 $ccount = mysqli_fetch_array(mysqli_query($link,('SELECT COUNT(id) as ccount  FROM requestbench  where jobid = "'.$jid.'"')));
...
//$submittedsql="select count(id) from submitted_benchcandidates where status='Awaiting Feedback' and jobid in(select jobid from benchjobs where memberid='$mid') and submitdate>=(select lastlogin from members where memberid='$mid')";
...
if(@mysqli_num_rows($jobres)){ 
    ...
    while($row =mysqli_fetch_array($jobres)){ 
			...
			$jobarray[$i]['requests']='0';
			 $jobid=$row['jobid'];
			 $res=mysqli_query($link,"SELECT * FROM requestbench WHERE jobid='$jobid'");
...
$content="You have not posted any bench jobs yet. Click here to <a href=https://www.recruitinghub.com/employer/post-benchjob><b>Post a Bench Job</b></a> or choose <a href=https://www.recruitinghub.com/employer/postdashboard><b>Recruiter Plans</b></a>";
?>
<!doctype html>
<html>
<head>
    ...
    <meta name="viewport" content="width=device-width, initial-scale=1">
     <title>Active Bench Jobs | Employers Hub</title>
...
<div class="account-navi-content">
  ...
  <div class="row">
    <div class="col-xs-12">
                    ...
                    <a class="btn btn-primary" href="post-benchjob">
    <i class="fa fa-plus"></i> Post Bench Job
</a>
      
      <div class="panel panel-default">
      <header class="panel-heading wht-bg ui-sortable-handle">
    <h3 class="panel-title"> List of Active Bench Jobs | <span class="d-inline-block" tabindex="0" data-toggle="tooltip" title="Click Action and Edit Job so it can be shown at top of recruiters marketplace">Not Receiving Enough CV's <i class="fa fa-question-circle"></i>
    </span> | <span class="d-inline-block" tabindex="0" data-toggle="tooltip" title="Click View Agencies -> Select All & Click Action -> Message Agency -> Type your Message and send so it can be sent to all engaged agencies">Want to communicate to all IT Vendors at once? <i class="fa fa-question-circle"></i>
    </span> </h3>
  </header>
  
  <div class="panel-body">
        <div class="text-left">
 Go to -> <a href="post-benchjob" id="one">POST NEW BENCH JOB</a> -> <a href="benchjobs" id="two"><strong>ACTIVE BENCH JOBS</strong></a></a> -> <a href="inactive" id="three">INACTIVE (HOLD)</a> -> <a href="closed" id="four">CLOSED</a> -> <a href="filled" id="five">FILLED</a> || <a href="all" id="six">ALL CANDIDATES</a>
</div>
        ...
        <div class="panel-body">
          ...
          <div class="row">
            <div class="col-md-12 table-responsive">
              <?php if(!$content){ ?>
              <div class="col-sm-12">
             <form method="GET" action="benchjobs" >
       <div class="form-group">
        
        <!--<label>Search Active Bench Jobs</label><br><br>-->
        <div class="col-sm-4">
        ...
        <div class="form-group">
              ...
              <h3>Total Active Bench Jobs:<?php echo $total; ?></h3>
...
$checkpriority=$row['priority'];
  ...
  <td><a class="title" href="#" data-toggle="modal" data-target="#engage-modal" id='.$row['jobid'].'>'.$row['jobtitle'].'</a><br>';
   ...
   <small>'.$row['joblocation'].'</small> | <small>'.$row['jobid'].'</small> | ';	
  	 {echo date("j F, Y",strtotime($row['updatedate']));}
  	echo ' </td>
   
 <td><b><a href="/employer/benchall?jobtitle='.urlencode($row['jobtitle']).'&candidate=&agency=&status=&submit=Search" >'.subcount($link,$row['jobid']).' Candidates</a></b></td>';
...
if($adminrights){
 ...
 echo'
     ...
     <td><b><a class="title" href="#" data-toggle="modal" data-target="#view-modal" id='.$row['jobid'].'>'.$row['requests'].' IT Vendors</b></a></td>
     <td>'.$row['country'].'</td>
...
<div class="modal fade" id="engage-modal" tabindex="-1" role="dialog" aria-hidden="true">
    ...
    $('#engage-modal').on('show.bs.modal', function(e) {
        ...
        $.ajax({
            cache: false,
            type: 'POST',
            url: 'engage-benchmodal-content.php',
            data: 'EID=' + essayId,
