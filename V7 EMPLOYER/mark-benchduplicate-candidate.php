...
if(isset($_GET['candidateid']) || isset($_POST['allcheck'])){
	...
	mysqli_query($link,"Update jobs INNER JOIN submitted_candidates ON submitted_candidates.jobid = jobs.jobid set submitted_candidates.status='Duplicate',submitted_candidates.feedbackdate=now() WHERE submitted_candidates.candidateid='$candidateid' and jobs.memberid='$mid'");
	$q2=mysqli_query($link,"select e.email,e.duplicatenotification,d.name,e.firstname from companyprofile d,members e ,submitted_benchcandidates c where  d.id=e.companyid  and e.memberid=c.recruiterid and c.candidateid='$candidateid'");
		while($row2 = mysqli_fetch_array($q2)){
			$array2=$row2;
				}
		$q1=mysqli_query($link,"select a.jobtitle,d.name from benchjobs a,companyprofile d,members e ,submitted_benchcandidates c where  d.id=e.companyid  and e.memberid=a.memberid and c.jobid=a.jobid and c.candidateid='$candidateid'  ");
		...
		$content="Candidate is marked as duplicate";
		$duplicatenotification=$array2['duplicatenotification'];
		...
		$to = $array2['email'];
			...
			$subject = "RecruitingHub.com - ".$array3['fname']." - Duplicate Notification for ".$array1['jobtitle']." from ".$array1['name']."   ";
                                      ...
                                      <tr>
                                        <td><p>
                                         Dear '.$array2['firstname'].',<br>
                                          <br>
                                          Your Candidate <strong>'.$array3['fname'].'</strong> submitted for <strong>'.$array1['jobtitle'].'</strong> role from <strong>'.$array1['name'].' </strong>has been marked as <b>Duplicate</b> by the Client. <br>
                                          <br>
                                          <b>Please check with the candidate again. If you already have taken RTR consent from candidate or if found not duplicate please use messaging system to ask client for further clarification on why the cv has been marked as duplicate. Please be polite to the client while checking about duplicacy.</b></p>  Login here <a href="https://www.recruitinghub.com/recruiter/login" target="_blank">Agency Login</a></td>
                                      </tr>
                                      <tr>
                                        <td><p>&nbsp;</p></td>
                                      </tr>
...
'subject'=>$subject,'message'=>$message,'cc'=>$r['email'],'send'=>true];
  ...
  echo "<script>
alert('Candidate Marked as Duplicate. An email notification has now been sent to the Recruiter');
</script>";
  ...
  <a href="#candidatesDropdownMenu1" data-toggle="collapse" aria-controls="candidatesDropdownMenu1" title="Candidates" class="navbar-root your-candidates">
   ...
   <ul class="nav-sub" style="display: block;">
    ...
    <li><a href="rejected" role="tab" > <i class="fa fa-trash"></i> Rejected</a> </li>
     ...
     <li><a href="msg" role="tab" ><i class="fa fa-fw fa-envelope-o"></i> Messages (<?php echo $new; ?>)</a>    </li>
