...
include "../config.php";
$jobtitleres=$recruiterres=$candidateres='';
if(isset($_GET)){
if(isset($_GET['jobtitle']) && ($_GET['jobtitle']!='')){
$jobtitle=$_GET['jobtitle'];
$jobtitleres="and a.jobtitle='$jobtitle'";
}
...
header("Content-Disposition: attachment; filename=awaitingfeedbackcandidates.xls");  
...
print("\n");    
        ...
        for($j=0; $j<mysqli_num_fields($result);$j++)
        {
            ...
            elseif ($row[$j] != "")
                ...
                $schema_insert .= "".$sep;
                 ...
                 if($j == count($row) - 1){
                $schema_insert.= resumeLink($row[3],$row[1],$row[$j]);
            }
              
           // $schema_insert.= resumeLink();
        }
    ...
    function resumeLink($rid, $cid, $cv){
    $base_url="https://".$_SERVER['SERVER_NAME'].'/';

     return $base_url.'recruiter/candidatecvs/'.$rid.'/'.$cid.'/'.$cv;
}
?>
