...
$sql="SELECT * FROM benchjobs WHERE status not in('Hold','Closed','Filled') and $block  filledvacancy < novacancies and jobid not in (select a.jobid from members f,companyprofile g,requestbench a where f.companyid=g.id and a.recruiterid=f.memberid and g.id='$cid' ) and updatedate>=CURDATE() - INTERVAL 2 MONTH order by updatedate desc";
...
if(($keywords) && ($sector) && ($type) && ($location) && ($jobid) && ($status)){
 	$param='keywords='.$keywords.'&sector='.$sector.'&type='.$type.'&location='.$location.'&jobid='.$jobid.'&status='.$status;
			...
			$sql = "SELECT * FROM benchjobs WHERE (jobtitle like '%$keywords%' ||  keyskills like '%$keywords%' ||  description like '%$keywords%')  AND jobsector='$sector' AND jobtype= '$type' AND $block  joblocation like '%$location%' AND jobid='$jobid' and status='$status' and status !='Closed' and filledvacancy < novacancies and jobid not in (select a.jobid from members f,companyprofile g,requestbench a where f.companyid=g.id and a.recruiterid=f.memberid and g.id='$cid')  and updatedate>=CURDATE() - INTERVAL 2 MONTH order by updatedate desc";
	}
...
elseif((!$keywords) && ($sector) && ($type) && ($location) && ($jobid) && ($status) ){
 	$param='sector='.$sector.'&type='.$type.'&location='.$location.'&jobid='.$jobid.'&status='.$status;
			...
			$sql = "SELECT * FROM benchjobs WHERE jobsector='$sector' AND jobtype= '$type' AND joblocation like '%$location%' AND $block  jobid='$jobid' and status='$status' and filledvacancy < novacancies and jobid not in (select a.jobid from members f,companyprofile g,requestbench a where f.companyid=g.id and a.recruiterid=f.memberid and g.id='$cid')  and updatedate>=CURDATE() - INTERVAL 2 MONTH order by updatedate desc";
	}
elseif((!$keywords) && (!$sector) && (!$type) && (!$location) && ($jobid) && (!$status) ){
 	$param='jobid='.$jobid;

			$sql = "SELECT * FROM benchjobs WHERE $block  jobid='$jobid' and filledvacancy < novacancies and jobid not in (select a.jobid from members f,companyprofile g,requestbench a where f.companyid=g.id and a.recruiterid=f.memberid and g.id='$cid')  and updatedate>=CURDATE() - INTERVAL 2 MONTH order by updatedate desc";
	}
elseif(($keywords) && (!$sector) && ($type) && ($location) && ($status) ){
 			$param='keywords='.$keywords.'&type='.$type.'&location='.$location.'&status='.$status;

			$sql = "SELECT * FROM benchjobs WHERE $block  (jobtitle like '%$keywords%' ||  keyskills like '%$keywords%' ||  description like '%$keywords%')  AND jobtype= '$type' AND joblocation like '%$location%' and status ='$status' and filledvacancy < novacancies and jobid not in (select a.jobid from members f,companyprofile g,requestbench a where f.companyid=g.id and a.recruiterid=f.memberid and g.id='$cid')  and updatedate>=CURDATE() - INTERVAL 2 MONTH order by updatedate desc";
	}
elseif(($keywords) && ($sector) && (!$type) && ($location) && ($status) ){
 			$param='keywords='.$keywords.'&sector='.$sector.'&location='.$location.'&status='.$status;

			$sql = "SELECT * FROM benchjobs WHERE $block  (jobtitle like '%$keywords%' ||  keyskills like '%$keywords%' ||  description like '%$keywords%')  AND jobsector='$sector' AND joblocation like '%$location%' and status ='$status' and filledvacancy < novacancies and jobid not in (select a.jobid from members f,companyprofile g,requestbench a where f.companyid=g.id and a.recruiterid=f.memberid and g.id='$cid')  and updatedate>=CURDATE() - INTERVAL 2 MONTH order by updatedate desc";
	}
elseif(($keywords) && ($sector) && ($type) && (!$location)  && ($status) ){
 			$param='keywords='.$keywords.'&sector='.$sector.'&type='.$type.'&status='.$status;

			$sql = "SELECT * FROM benchjobs WHERE $block  (jobtitle like '%$keywords%' ||  keyskills like '%$keywords%' ||  description like '%$keywords%')  AND jobsector='$sector' AND jobtype= '$type' and status='$status' and filledvacancy < novacancies and jobid not in (select a.jobid from members f,companyprofile g,requestbench a where f.companyid=g.id and a.recruiterid=f.memberid and g.id='$cid')  and updatedate>=CURDATE() - INTERVAL 2 MONTH order by updatedate desc";
	}
elseif(($keywords) && ($sector) && ($type) && ($location)  && (!$status) ){
 			$param='keywords='.$keywords.'&sector='.$sector.'&type='.$type.'&location='.$location;

			$sql = "SELECT * FROM benchjobs WHERE $block  (jobtitle like '%$keywords%' ||  keyskills like '%$keywords%' ||  description like '%$keywords%')  AND jobsector='$sector' AND joblocation like '%$location%' AND jobtype= '$type' and  status not in('Hold','Closed','Filled') and filledvacancy < novacancies and jobid not in (select a.jobid from members f,companyprofile g,requestbench a where f.companyid=g.id and a.recruiterid=f.memberid and g.id='$cid')  and updatedate>=CURDATE() - INTERVAL 2 MONTH order by updatedate desc";
	}
	
	
elseif((!$keywords) && (!$sector) && ($type) && ($location) &&($status) ){
 			$param='type='.$type.'&location='.$location.'&status='.$status;
			$sql = "SELECT * FROM benchjobs WHERE $block  jobtype= '$type' AND joblocation like '%$location%' and status='$status' and filledvacancy < novacancies and jobid not in (select a.jobid from members f,companyprofile g,requestbench a where f.companyid=g.id and a.recruiterid=f.memberid and g.id='$cid')  and updatedate>=CURDATE() - INTERVAL 2 MONTH order by updatedate desc";
	}
elseif(($keywords) && (!$sector) && (!$type) && ($location) &&($status) ){
 			$param='keywords='.$keywords.'&location='.$location.'&status='.$status;
			$sql = "SELECT * FROM benchjobs WHERE $block  (jobtitle like '%$keywords%' ||  keyskills like '%$keywords%' ||  description like '%$keywords%') AND joblocation like '%$location%' and status='$status' and filledvacancy < novacancies and jobid not in (select a.jobid from members f,companyprofile g,requestbench a where f.companyid=g.id and a.recruiterid=f.memberid and g.id='$cid')  and updatedate>=CURDATE() - INTERVAL 2 MONTH order by updatedate desc";
	}
elseif(($keywords) && ($sector) && (!$type) && (!$location) &&($status) ){
 			$param='keywords='.$keywords.'&sector='.$sector.'&status='.$status;
			$sql = "SELECT * FROM benchjobs WHERE $block  (jobtitle like '%$keywords%' ||  keyskills like '%$keywords%' ||  description like '%$keywords%')  AND jobsector='$sector' AND status='$status' and filledvacancy < novacancies and jobid not in (select a.jobid from members f,companyprofile g,requestbench a where f.companyid=g.id and a.recruiterid=f.memberid and g.id='$cid')  and updatedate>=CURDATE() - INTERVAL 2 MONTH order by updatedate desc";
	}
elseif((!$keywords) && ($sector) && ($type) && (!$location) &&($status) ){
 			$param='sector='.$sector.'&type='.$type.'&status='.$status;
			$sql = "SELECT * FROM benchjobs WHERE $block  jobsector='$sector' AND jobtype= '$type' AND  status='$status' and filledvacancy < novacancies and jobid not in (select a.jobid from members f,companyprofile g,requestbench a where f.companyid=g.id and a.recruiterid=f.memberid and g.id='$cid')  and updatedate>=CURDATE() - INTERVAL 2 MONTH order by updatedate desc";
	}
elseif((!$keywords) && ($sector) && (!$type) && ($location) &&($status) ){
 			$param='sector='.$sector.'&location='.$location.'&status='.$status;
			$sql = "SELECT * FROM benchjobs WHERE $block  jobsector='$sector'  AND joblocation like '%$location%' and status='$status' and filledvacancy < novacancies and jobid not in (select a.jobid from members f,companyprofile g,requestbench a where f.companyid=g.id and a.recruiterid=f.memberid and g.id='$cid')  and updatedate>=CURDATE() - INTERVAL 2 MONTH order by updatedate desc";
	}
elseif(($keywords) && (!$sector) && ($type) && (!$location) &&($status)){
 			$param='keywords='.$keywords.'&type='.$type.'&status='.$status;
			$sql = "SELECT * FROM benchjobs WHERE $block  (jobtitle like '%$keywords%' ||  keyskills like '%$keywords%' ||  description like '%$keywords%')  AND jobtype= '$type' AND status='$status' and filledvacancy < novacancies and jobid not in (select a.jobid from members f,companyprofile g,requestbench a where f.companyid=g.id and a.recruiterid=f.memberid and g.id='$cid')  and updatedate>=CURDATE() - INTERVAL 2 MONTH order by updatedate desc";
	}
elseif((!$keywords) && ($sector) && ($type) && ($location) &&(!$status)){
 			$param='sector='.$sector.'&type='.$type.'&location='.$location;
			$sql = "SELECT * FROM benchjobs WHERE  $block  jobsector='$sector' AND jobtype= '$type' AND joblocation like '%$location%' and status not in('Hold','Closed','Filled') and filledvacancy < novacancies and jobid not in (select a.jobid from members f,companyprofile g,requestbench a where f.companyid=g.id and a.recruiterid=f.memberid and g.id='$cid')  and updatedate>=CURDATE() - INTERVAL 2 MONTH order by updatedate desc";
	}
elseif(($keywords) && ($sector) && ($type) && (!$location) && (!$status)){
 			$param='keywords='.$keywords.'&sector='.$sector.'&type='.$type;
			$sql = "SELECT * FROM benchjobs WHERE $block  (jobtitle like '%$keywords%' ||  keyskills like '%$keywords%' ||  description like '%$keywords%')  AND jobsector='$sector' AND jobtype= '$type' and status not in('Hold','Closed','Filled') and filledvacancy < novacancies and jobid not in (select a.jobid from members f,companyprofile g,requestbench a where f.companyid=g.id and a.recruiterid=f.memberid and g.id='$cid')  and updatedate>=CURDATE() - INTERVAL 2 MONTH order by updatedate desc";
	}
elseif(($keywords) && (!$sector) && ($type) && ($location) && (!$status)){
 			$param='keywords='.$keywords.'&type='.$type.'&location='.$location;
			$sql = "SELECT * FROM benchjobs WHERE $block  (jobtitle like '%$keywords%' ||  keyskills like '%$keywords%' ||  description like '%$keywords%')   AND jobtype= '$type' AND joblocation like '%$location%' and  filledvacancy < novacancies and jobid not in (select a.jobid from members f,companyprofile g,requestbench a where f.companyid=g.id and a.recruiterid=f.memberid and g.id='$cid')  and updatedate>=CURDATE() - INTERVAL 2 MONTH order by updatedate desc";
	}
elseif(($keywords) && ($sector) && (!$type) && ($location) && (!$status)){
 			$param='keywords='.$keywords.'&sector='.$sector.'&location='.$location;
			$sql = "SELECT * FROM benchjobs WHERE $block (jobtitle like '%$keywords%' ||  keyskills like '%$keywords%' ||  description like '%$keywords%')  AND jobsector='$sector' AND joblocation like '%$location%' and   filledvacancy < novacancies and jobid not in (select a.jobid from members f,companyprofile g,requestbench a where f.companyid=g.id and a.recruiterid=f.memberid and g.id='$cid')  and updatedate>=CURDATE() - INTERVAL 2 MONTH order by updatedate desc";
	}
...
elseif(($keywords) && (!$sector) && (!$type) && (!$location) &&($status) ){
			...
			$sql = "SELECT * FROM benchjobs WHERE $block  (jobtitle like '%$keywords%' ||  keyskills like '%$keywords%' ||  description like '%$keywords%') and status='$status' and filledvacancy < novacancies and jobid not in (select a.jobid from members f,companyprofile g,requestbench a where f.companyid=g.id and a.recruiterid=f.memberid and g.id='$cid')  and updatedate>=CURDATE() - INTERVAL 2 MONTH order by updatedate desc";
	}
elseif((!$keywords) && ($sector) && (!$type) && (!$location) &&($status) ){
 			$param='sector='.$sector.'&status='.$status;
			$sql = "SELECT * FROM benchjobs WHERE $block  jobsector='$sector' and status='$status' and filledvacancy < novacancies and jobid not in (select a.jobid from members f,companyprofile g,requestbench a where f.companyid=g.id and a.recruiterid=f.memberid and g.id='$cid')  and updatedate>=CURDATE() - INTERVAL 2 MONTH order by updatedate desc";
	}
elseif((!$keywords) && (!$sector) && ($type) && (!$location) &&($status) ){
 			$param='type='.$type.'&status='.$status;
			$sql = "SELECT * FROM benchjobs WHERE $block  jobtype= '$type' and status='$status' and filledvacancy < novacancies and jobid not in (select a.jobid from members f,companyprofile g,requestbench a where f.companyid=g.id and a.recruiterid=f.memberid and g.id='$cid')  and updatedate>=CURDATE() - INTERVAL 2 MONTH order by updatedate desc";
	}
elseif((!$keywords) && (!$sector) && (!$type) && ($location) &&($status) ){
 			$param='location='.$location.'&status='.$status;
			$sql = "SELECT * FROM benchjobs WHERE $block  (joblocation like '%$location%' and status='$status') and filledvacancy < novacancies and jobid not in (select a.jobid from members f,companyprofile g,requestbench a where f.companyid=g.id and a.recruiterid=f.memberid and g.id='$cid')  and updatedate>=CURDATE() - INTERVAL 2 MONTH order by updatedate desc";
	}	
elseif(($keywords) && ($sector) && (!$type) && (!$location) && (!$status)){
 			$param='keywords='.$keywords.'&sector='.$sector;
			$sql = "SELECT * FROM benchjobs WHERE $block  (jobtitle like '%$keywords%' ||  keyskills like '%$keywords%' ||  description like '%$keywords%')  AND jobsector='$sector' and status not in('Hold','Closed','Filled') and filledvacancy < novacancies and jobid not in (select a.jobid from members f,companyprofile g,requestbench a where f.companyid=g.id and a.recruiterid=f.memberid and g.id='$cid')  and updatedate>=CURDATE() - INTERVAL 2 MONTH order by updatedate desc";
	}
elseif(($keywords) && (!$sector) && ($type) && (!$location) && (!$status)){
 			$param='keywords='.$keywords.'&type='.$type;
			$sql = "SELECT * FROM benchjobs WHERE $block  (jobtitle like '%$keywords%' ||  keyskills like '%$keywords%' ||  description like '%$keywords%')  AND  jobtype= '$type' and status not in('Hold','Closed','Filled')  and filledvacancy < novacancies and jobid not in (select a.jobid from members f,companyprofile g,requestbench a where f.companyid=g.id and a.recruiterid=f.memberid and g.id='$cid')  and updatedate>=CURDATE() - INTERVAL 2 MONTH  order by updatedate desc";
	}	
elseif(($keywords) && (!$sector) && (!$type) && ($location) && (!$status)){
 			$param='keywords='.$keywords.'&location='.$location;
			$sql = "SELECT * FROM benchjobs WHERE $block  (jobtitle like '%$keywords%' ||  keyskills like '%$keywords%' ||  description like '%$keywords%')    AND joblocation like '%$location%' and status not in('Hold','Closed','Filled') and filledvacancy < novacancies and jobid not in (select a.jobid from members f,companyprofile g,requestbench a where f.companyid=g.id and a.recruiterid=f.memberid and g.id='$cid')  and updatedate>=CURDATE() - INTERVAL 2 MONTH order by updatedate desc";
	}	
elseif((!$keywords) && ($sector) && ($type) && (!$location) && (!$status)){
 			$param='sector='.$sector.'&type='.$type;
			$sql = "SELECT * FROM benchjobs WHERE $block  jobsector='$sector' AND jobtype= '$type' and status not in('Hold','Closed','Filled') AND filledvacancy < novacancies and jobid not in (select a.jobid from members f,companyprofile g,requestbench a where f.companyid=g.id and a.recruiterid=f.memberid and g.id='$cid')  and updatedate>=CURDATE() - INTERVAL 2 MONTH order by updatedate desc";
	}	
elseif((!$keywords) && ($sector) && (!$type) && ($location) && (!$status)){
 			$param='sector='.$sector.'&location='.$location;
			$sql = "SELECT * FROM benchjobs WHERE $block jobsector='$sector' AND  joblocation like '%$location%' and status not in('Hold','Closed','Filled') and filledvacancy < novacancies and jobid not in (select a.jobid from members f,companyprofile g,requestbench a where f.companyid=g.id and a.recruiterid=f.memberid and g.id='$cid')  and updatedate>=CURDATE() - INTERVAL 2 MONTH order by updatedate desc";
	}
elseif((!$keywords) && (!$sector) && ($type) && ($location) && (!$status)){
 			$param='type='.$type.'&location='.$location;
			$sql = "SELECT * FROM benchjobs WHERE $block  jobtype= '$type' AND joblocation like '%$location%' and status not in('Hold','Closed','Filled') and filledvacancy < novacancies and jobid not in (select a.jobid from members f,companyprofile g,requestbench a where f.companyid=g.id and a.recruiterid=f.memberid and g.id='$cid')  and updatedate>=CURDATE() - INTERVAL 2 MONTH order by updatedate desc";
	}
...
elseif(($keywords) && (!$sector) && (!$type) && (!$location) && (!$status)){
			...
			$sql = "SELECT * FROM benchjobs WHERE $block  (jobtitle like '%$keywords%' || keyskills like '%$keywords%' ||  description like '%$keywords%') and status not in('Hold','Closed','Filled')  and filledvacancy < novacancies and jobid not in (select a.jobid from members f,companyprofile g,requestbench a where f.companyid=g.id and a.recruiterid=f.memberid and g.id='$cid')  and updatedate>=CURDATE() - INTERVAL 2 MONTH order by updatedate desc";
	}	
elseif((!$keywords) && ($sector) && (!$type) && (!$location) && (!$status)){
 			$param='sector='.$sector;
			$sql = "SELECT * FROM benchjobs WHERE $block   jobsector='$sector' and status not in('Hold','Closed','Filled') and filledvacancy < novacancies and jobid not in (select a.jobid from members f,companyprofile g,requestbench a where f.companyid=g.id and a.recruiterid=f.memberid and g.id='$cid')  and updatedate>=CURDATE() - INTERVAL 2 MONTH order by updatedate desc";
	}
elseif((!$keywords) && (!$sector) && ($type) && (!$location) && (!$status)){
 			$param='type='.$type;
			$sql = "SELECT * FROM benchjobs WHERE $block  jobtype= '$type' and status not in('Hold','Closed','Filled') and filledvacancy < novacancies and jobid not in (select a.jobid from members f,companyprofile g,requestbench a where f.companyid=g.id and a.recruiterid=f.memberid and g.id='$cid')  and updatedate>=CURDATE() - INTERVAL 2 MONTH order by updatedate desc";
	}	
elseif((!$keywords) && (!$sector) && (!$type) && ($location) && (!$status)){
 			$param='location='.$location;
			$sql = "SELECT * FROM benchjobs WHERE  $block  joblocation like '%$location%' and status not in('Hold','Closed','Filled') and filledvacancy < novacancies and jobid not in (select a.jobid from members f,companyprofile g,requestbench a where f.companyid=g.id and a.recruiterid=f.memberid and g.id='$cid')  and updatedate>=CURDATE() - INTERVAL 2 MONTH order by updatedate desc";
	}	
elseif((!$keywords) && (!$sector) && (!$type) && (!$location) && ($status)){
 			$param='status='.$status;
			$sql = "SELECT * FROM benchjobs WHERE $block   status='$status' and filledvacancy < novacancies and jobid not in (select a.jobid from members f,companyprofile g,requestbench a where f.companyid=g.id and a.recruiterid=f.memberid and g.id='$cid')  and updatedate>=CURDATE() - INTERVAL 2 MONTH order by updatedate desc";
	}	

							
else{
$sql="SELECT * FROM benchjobs WHERE $block status not in('Hold','Closed','Filled') and filledvacancy < novacancies and jobid not in (select a.jobid from members f,companyprofile g,requestbench a where f.companyid=g.id and a.recruiterid=f.memberid and g.id='$cid') and updatedate>=CURDATE() - INTERVAL 2 MONTH order by updatedate desc";
...
if(!$rcount)
$content="No bench jobs for you to engage. ";
...
<div class="account-navi-content">
  ...
  <div class="row">
    ...
    <h3 class="panel-title"><a>BENCH JOBS POSTED BY CLIENTS: </a> <a href="searchbenchjobs"><strong>All</strong></a>  </h3>
  </header>
        <div class="panel-body">
          <div class="row results-table-filter">
            <form id="job_filter_form" class="job_search" action="searchbenchjobs" accept-charset="UTF-8" method="GET">
              <div class="col-md-12">
                <div class="row">
                  <div class="col-md-2">
                ...
                <div class="row">
                 ...
                 <div class="col-md-2">
                  ...
                  <a class="btn btn-primary" href="engagedbenchjob">
    <i class="fa fa-forward"></i>  Engaged Bench Jobs
</a> 
             
             
</form>         
 </div>
 <h3>Total Available Live Bench Roles: 🌎 <?php echo $total; ?></h3>
  ...
  <div class="row">
              <div class="col-md-12 table-responsive">
               <div class="panel panel-default">
                <header class="panel-heading wht-bg ui-sortable-handle">
    <h3 class="panel-title"> Bench Jobs posted by Clients (SUPPLY CANDIDATES AND EARN 2X FASTER)  </h3>
  </header>
        <div class="panel-body">
               <?php if(!$content){ ?>
...
$checkrpo=$row['rpo'];
     ...
     //<a class="title" target="_blank" href="view-profile?EID='.$row['jobid'].'" data-toggle="modal"  id='.$row['jobid'].'>'.$row['jobtitle'].'</a>';
                ...
                ';
						 // if(($checkonlinestatus!='0')) {echo' <span class="label label-primary">Employer Online Now</span>'; } else  {echo' <small>Last Login: '.$row['lastlogin'].'</small>'; } echo' ';
						  if(($checkpriority!='0')) {echo' <span class="label label-primary">Priority</span>'; } echo' 
						  <br>';	
  	 {echo date("j F, Y",strtotime($row['updatedate']));}
  	echo ' |  <small>Job ID: '.$row['jobid'].'</small> 
						 </td>
						 <td>';
						if(in_array($row['jobsector'],$mysector))
						echo '<a title="Engage" class="btn btn-primary" href="engagebenchjob?jobid='.$row['jobid'].'&employerid='.$row['memberid'].'">Engage</a>';
						else
						echo '<a title="Engage" class="btn btn-primary" href="engagebenchjob?jobid='.$row['jobid'].'&employerid='.$row['memberid'].'">Engage</a>';
						echo'
					  </td>
           ...
           success: function(data) {
               if (data.length > 0){  // if at least 1 is online
                  $('.status').each(function(){  // loop through each of the user posts
                      var memberid = $(this).attr('id'); // get the user#
                      if($.inArray(memberid, data) !== -1){  // if userid # in the returned data array set to online
