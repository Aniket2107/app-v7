...
$lastlogin="select * from members where lastlogin=$lastlogin";	

if(isset($_POST['EID']))

$jobid=$_POST['EID'];

$jobres = mysqli_query($link,"SELECT a.*,b.accountmanager,companyprofile.profile as companydescription,companyprofile.website as website,companyprofile.logo as logo,companyprofile.id as id,companyprofile.name as name,b.replacementperiod,b.specialnotes FROM benchjobs a,employers b,companyprofile,members WHERE a.jobid='$jobid' and a.memberid=members.memberid and members.companyid=companyprofile.id and companyprofile.registerid=b.id"); 
...
$ispresent = mysqli_num_rows($jobres);
if($ispresent > 0){ 
   $row = mysqli_fetch_array($jobres);
...
$isengaged=mysqli_num_rows(mysqli_query($link,"select * from requestbench where jobid='$jobid' and recruiterid='$mid' and status not in ('Requested','Rejected')"));

$requested=mysqli_num_rows(mysqli_query($link,"select * from requestbench where jobid='$jobid' and recruiterid='$mid' and status = 'Requested'"));
...
<div id="job_title" data-value=""></div><b><?php echo $job['name'];?></b> | <b><?php echo $job['jobtitle'];?></b> | Job ID: <b><?php echo $job['jobid'];?></b> | Job Status: <b><?php echo $job['status'];?></b> | 1st Posted: <?php echo date("j F, Y",strtotime($job['postdate']));  ?> | Last Updated: <b><?php echo date("j F, Y",strtotime($job['updatedate']));  ?></b></div>
...
<table class="table table-striped">
 ...
 <tr>
               ...
               <td>      <?php 
					 $companyprofile= htmlspecialchars_decode($job['companyprofile'],ENT_QUOTES);
					  if($job['logo'])
					 echo'
					 <img src="../employer/logo/'.$job['id'].'/'.$job['logo'].'" width="100px"  height="100px"/><br>
					 '; ?>
  ...
  <!--  <tr>
      ...
      <td><b><?php echo $job['name'];?></b> | <b><?php echo $job['jobtitle'];?></b> | Job ID: <b><?php echo $job['jobid'];?></b> | Job Status: <b><?php echo $job['status'];?></b> | 1st Posted: <?php echo date("j F, Y",strtotime($job['postdate']));  ?> | Last Updated: <b><?php echo date("j F, Y",strtotime($job['updatedate']));  ?></b> </td>
...
<?php 	if($row['requeststatus']!='Requested' &&$row['requeststatus']!='Rejected' &&$row['requeststatus']!='Disengaged' &&($isengaged)  && !in_array($job['status'],array('Closed','Hold'))) {   
   ...
   <tr>
      ...
      <td><?php echo '<a  target=\"_blank\" href="'.addhttp($job['website']).'">'.$job['website'].'</a>' ?> </td>
...
//  <br> <b>Employer Website</b>: '.$job['website'].' ';
?>
