...
$j=$_POST['jobids'];
	...
	$candidateid=$value;
	$namesql=mysqli_query($link,"select fname from benchcandidates where id='$candidateid'");
	$namerow=mysqli_fetch_assoc($namesql);




	$jobmemsql=mysqli_query($link,"select memberid from benchjobs where jobid=$j");
	...
	$res=mysqli_query($link,"select a.candidateid,a.status,a.recruiterid,b.jobtitle,a.jobid from submitted_benchcandidates a,benchjobs b WHERE a.candidateid='$candidateid' and a.jobid=b.jobid and b.memberid='$jmid' and a.jobid=$j");
...
elseif(isset($_POST['submit'])){
	...
	mysqli_query($link,"Update jobs INNER JOIN submitted_candidates ON submitted_candidates.jobid = jobs.jobid set submitted_candidates.status='Rejected',submitted_candidates.feedbackdate=now(), submitted_candidates.comment='$comment',submitted_candidates.prestatus='$prestatus[$i]' WHERE submitted_candidates.candidateid='$candidateid' and jobs.memberid='$jmid' and submitted_candidates.jobid='$fetchjid'");
			...
			$namesql1=mysqli_query($link,"select fname,email from benchcandidates where id='$candidateid'");
	...
	$namerow2=mysqli_fetch_assoc($namesql1);
			$fetchemail2=mysqli_fetch_assoc(mysqli_query($link,"select email from benchcandidates where id='$candidateid'"));
			$fetchname2=mysqli_fetch_assoc(mysqli_query($link,"select fname from benchcandidates where id='$candidateid'"));
...
<table style="border-right:1px solid #d5d8d9;border-left:1px solid #d5d8d9;font-family:Arial,Helvetica,sans-serif;font-size:12px;color:#000" align="center" bgcolor="#792785" border="0" cellpadding="0" cellspacing="0" width="620">
  ...
  <tr>
    ...
    <table style="border-right:1px solid #d5d8d9;border-left:1px solid #d5d8d9" bgcolor="#FFFFFF" border="0" cellpadding="0" cellspacing="0" width="100%">
      ...
      <div style="boder:0"><a href="https://www.recruitinghub.com/recruiter/login"><img src="https://www.recruitinghub.com/images/Rejected.png" alt="www.recruitinghub.com"></a></div>
      </td>
	  ...
	  <tr>
		...
		<p>Best Regards,</p>
		<p><a href="https://www.recruitinghub.com/recruiter/login">www.recruitinghub.com</a></p>
					                </td>                
                </tr>            
...
<table style="border-right:1px solid #d5d8d9;border-left:1px solid #d5d8d9;font-family:Arial,Helvetica,sans-serif;font-size:12px;color:#000" align="center" bgcolor="#792785" border="0" cellpadding="0" cellspacing="0" width="620">
  ...
  <tr>
    ...
    <table style="border-right:1px solid #d5d8d9;border-left:1px solid #d5d8d9" bgcolor="#FFFFFF" border="0" cellpadding="0" cellspacing="0" width="100%">
      ...
      <td colspan="3" style="padding-right:12px">
      <div style="boder:0"><a href="https://www.recruitinghub.com/"><img src="https://www.recruitinghub.com/images/Rejected.png" alt="www.recruitinghub.com"></a></div>
       </tr>
	  <tr>
        <td style="padding:0px 14px">
        <table border="0" cellpadding="0" cellspacing="0" width="100%">
