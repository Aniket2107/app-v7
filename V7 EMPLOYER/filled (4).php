...
mysqli_query($link,"Update candidates SET totalinv='$invoice',tax='$tax',billable='$bamt',net='$net',rhshare='$rhshare',chanshare='$chanshare',agencyshare='$agencyshare' WHERE id=$cid ");
	...
	$q2=mysqli_query($link,"select e.email,e.fillnotification,d.name,e.firstname from companyprofile d,members e ,submitted_candidates c where  d.id=e.companyid  and e.memberid=c.recruiterid and c.candidateid='$candidateid'");
		...
		$q1=mysqli_query($link,"select a.jobtitle,a.fee,d.name,e.email,e.firstname from jobs a,companyprofile d,members e ,submitted_candidates c where  d.id=e.companyid  and e.memberid=a.memberid and c.jobid=a.jobid and c.candidateid='$candidateid'  ");
...
//send invoice to Client
   		...
   		$to = $email;
			...
			$from = "accounts@recruitinghub.com";

            $r=	getAccountManagerDetail($link,$mid);

		$subject = "RecruitingHub.com - Placement fee INVOICE for - $id ";
	...
	<table style="border-right:1px solid #d5d8d9;border-left:1px solid #d5d8d9;font-family:Arial,Helvetica,sans-serif;font-size:12px;color:#000" align="center" bgcolor="#792785" border="0" cellpadding="0" cellspacing="0" width="620">
	  ...
	  <tr>
		...
		<table style="border-right:1px solid #d5d8d9;border-left:1px solid #d5d8d9" bgcolor="#FFFFFF" border="0" cellpadding="0" cellspacing="0" width="100%">
		  ...
		  <div style="boder:0"><a href="https://www.recruitinghub.com/"><img src="https://www.recruitinghub.com/image/INVOICE.png" alt="www.recruitinghub.com"></a></div>
		  </td>
			...
			<table border="0" cellpadding="0" cellspacing="0" width="100%">
			  <tbody>
					   <tr>
						...
						<br><br>
						We thank you for your business. Please find below your invoice details. Please email us within 2 days from receipt of this invoice if all details in the invoice are correct post which this invoice will be sent to our collections team for processing. If your GST number is not already included and needs to be included in the invoice please email us asap with your GST number for filing.
						  <br><br>
						 
						  <b>INVOICE details</b>:<br><br>
						  
						  <b>From:</b><br>
						  Recruiting Hub India Pvt Ltd<br>
						  300 CTH Road, Avadi<br>
                          (opp Jak City)<br>
                          Chennai Tamil Nadu 600054 | India<br>
                          GSTIN - 33AAKCR5638L1ZX<br><br>
                          
                          <b>To:</b><br>
						  '.$name.'<br>
						  '.$uclientaddress.'<br>
						   Client GSTIN: '.$uclientgst.'<br><br>
						  
						  <b>For:</b><br>
						  Candidate Name: <b>'.$array3['fname'].'</b><br>
						  Description: One time placement fee invoice<br>
						  HSN Code: 998517<br>
						  DOJ: '.$doj.'<br>
						  Billable/Offer Amount: '.$bamt.'<br>
						  Billing Fee: '.$fee.'<br>
						  Net Rate: '.$net.'<br>
						  GST 18 %: '.$tax.'<br>
<p style="font-size: 14px;color: #a02121;">Total Invoice: <b>'.$invoice.'</b></p>

<p style="font-size: 14px;color: #a02121;">Invoice No: <b>'.$id.'</b></p>

						  <p>Invoice Date: '.$uinvdate.'</p>
<p style="font-size: 14px;color: #a02121;">Due Date: <b>'.$uduedate.'</b></p> <br>

                          <b>Pay to:</b><br>
		 Beneficiary Name: <b>Recruiting Hub India Pvt Ltd</b><br>
						  Bank: <b>Axis Bank</b><br>
						  A/c No: <b>920020072841887</b><br>
						  IFSC Code: <b>UTIB0001543</b><br>
						  Company PAN: <b>AAKCR5638L</b><br>
						  
						  <br>
						  
	 <a href="https://www.recruitinghub.com/bankdetails" target="_blank"><b>Click here for all our Company details</b></a> 
 ...
 $params = ['from'=>$from,'name'=>'Recruiting Hub INVOICE','to'=>$to,'cc'=>$r['email'],'cc2'=>'accounts@recruitinghub.com','subject'=>$subject,'message'=>$message1,'send'=>true];
   AsyncMail($params);
...
</a> </li> <a class="btn btn-primary" href="https://www.recruitinghub.com/bankdetails" target=_blank> RH Bank Details </a>   <a class="btn btn-primary" href="edit-company" target=_blank> Update Company Details </a>      
      <div class="panel panel-default">
      <header class="panel-heading wht-bg ui-sortable-handle">
    <h3 class="panel-title"> List of Filled Jobs <span class="d-inline-block" tabindex="0" data-toggle="tooltip" title="Click on DOJ - Update DOJ Date box and enter the date of candidate joining and click on Billable - Update Offer Amount box and enter what you have offered the candidate so we can send you an Invoice">(Update Billing) <i class="fa fa-question-circle"></i>
    </span> </h3>
  </header>
...
$rfee=$row['fee'];
	...
	$jtype =   $row['jobtype'];
			...
			<small>'.$row['joblocation'].'</small> | <small>'.$row['jobid'].'</small> / <small>'.$row['jobtype'].'</small> | ';	
  	...
  	echo ' </td>
			<td><b><a class="title" href="#" data-toggle="modal" data-target="#candidate-modal" id='.$row['candidateid'].'>'.$row['candidatename'].'</a></b><br>					  
				<small>Invoice No.'.$row['candidateid'].'</small></td>
			<td> <b>';	
  	 {echo date("j F, Y",strtotime($row['doj']));}
  	echo '</b> <br><input class="datepicker" name="doj" onChange="dojOnChange( this.value,'.$row['candidateid'].' )" placeholder="Update DOJ Date" type="text" /> </td> 
		<td><b>'.$row['currentcurrency'].'</b><b>'.$row['billable'].'</b> <br>
			...
			<td>'.$row['recruiter'].'</td>
			<td><a class="btn btn-primary" href="https://www.recruitinghub.com/bankdetails" target=_blank> RH Bank Details </a>Invoice No.'.$row['candidateid'].'</td>';
      ...
      if(fee!= null){
         ...
         invoice = net + tax
          ...
          return false;
            ...
            success: function(data) {
alert('Offer Details Updated & an INVOICE has been sent to your email id');
window.location.reload();
