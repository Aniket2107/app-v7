...
$submittedsql="select count(id) from submitted_benchcandidates where status='Awaiting Feedback' and jobid in(select jobid from benchjobs where memberid='$mid') and submitdate >= CURDATE()-INTERVAL 6 MONTH";
...
$submittedsql1="select count(id) from submitted_benchcandidates where status='Shortlisted' and jobid in(select jobid from benchjobs where memberid='$mid') and submitdate >= CURDATE()-INTERVAL 6 MONTH";
...
$submittedsql2="select count(id) from submitted_benchcandidates where status='Offered' and jobid in(select jobid from benchjobs where memberid='$mid') and submitdate >= CURDATE()-INTERVAL 6 MONTH";
...
$submittedsql3="select count(id) from submitted_benchcandidates where status='Rejected' and jobid in(select jobid from benchjobs where memberid='$mid') and submitdate >= CURDATE()-INTERVAL 6 MONTH";
...
$submittedsql4="select count(id) from submitted_benchcandidates where status='Filled' and jobid in(select jobid from benchjobs where memberid='$mid') and submitdate >= CURDATE()-INTERVAL 12 MONTH";
...
$_SESSION['mark']=$_GET['candidate'];
	...
	if($adminrights){
    $sql="SELECT a.jobtitle as jobtitle,a.jobid,d.email as recruiteremail,e.name as agency,e.registerid, b.fname as candidatename,b.contactnumber,b.cv as cv,b.resume,c.status as candidatestatus,c.prestatus,c.viewnotes,c.comment,b.jobtype,b.desiredsalary,b.expectedcurrency,b.typesalary,c.submitdate,c.candidateid as candidateid, c.recruiterid as recruiterid , c.viewed,c.viewedtime,s.firstname as subuser,d.onlinestatus as onlinestatus,d.lastlogin as lastlogin,d.firstname as recsubuser,d.email as subuseremail FROM submitted_benchcandidates c,benchjobs a,benchcandidates b ,companyprofile e,members d ,members s  WHERE s.companyid ='$companyid' and s.memberid = a.memberid and a.jobid=c.jobid and b.id=c.candidateid and a.jobtitle like '%$jobtitle%' and ( b.resume like '%$candidate%' or c.candidateid='$candidate') and c.recruiterid= d.memberid and d.companyid=e.id and e.name ='$agency' and $filter order by submitdate desc";

  }else{
    $sql="SELECT a.jobtitle as jobtitle,a.jobid,d.email as recruiteremail,e.name as agency,e.registerid, b.fname as candidatename,b.contactnumber,b.cv as cv,b.resume,c.status as candidatestatus,c.prestatus,c.viewnotes,c.comment,b.jobtype,b.desiredsalary,d.onlinestatus as onlinestatus,d.lastlogin as lastlogin,b.expectedcurrency,b.typesalary,c.submitdate,c.candidateid as candidateid, c.recruiterid as recruiterid , c.viewed,c.viewedtime FROM submitted_benchcandidates c,benchjobs a,benchcandidates b ,companyprofile e,members d WHERE a.memberid='$mid' and a.jobid=c.jobid and b.id=c.candidateid and a.jobtitle like '%$jobtitle%' and ( b.resume like '%$candidate%' or c.candidateid='$candidate') and c.recruiterid= d.memberid and d.companyid=e.id and e.name ='$agency' and $filter order by submitdate desc";
...
elseif(isset($_GET['jobowner']) && ($_GET['jobowner']!='')){
  ...
  $jobowner=$_GET['jobowner'];
    if($adminrights){
      $sql="SELECT a.jobtitle as jobtitle,a.jobid,d.email as recruiteremail,e.name as agency,e.registerid, b.fname as candidatename,b.contactnumber,b.cv as cv,b.resume,c.status as candidatestatus,c.prestatus,c.viewnotes,c.comment,b.jobtype,b.desiredsalary,b.expectedcurrency,b.typesalary,c.submitdate,d.onlinestatus as onlinestatus,d.lastlogin as lastlogin,c.candidateid as candidateid, c.recruiterid as recruiterid , c.viewed,c.viewedtime,s.firstname as subuser,d.firstname as recsubuser,d.email as subuseremail FROM submitted_benchcandidates c,benchjobs a,benchcandidates b ,companyprofile e,members d ,members s  WHERE s.memberid ='$jobowner' and s.memberid = a.memberid and a.jobid=c.jobid and b.id=c.candidateid and c.recruiterid= d.memberid and d.companyid=e.id and submitdate >= CURDATE()-INTERVAL 6 MONTH order by submitdate desc";
...
$candidate=$_GET['candidate'];
if($adminrights){
  $sql="SELECT a.jobtitle as jobtitle,a.jobid,d.email as recruiteremail,e.name as agency,e.registerid, b.fname as candidatename,b.contactnumber,b.cv as cv,b.resume,c.status as candidatestatus,c.prestatus,c.viewnotes,c.comment,b.jobtype,b.desiredsalary,d.onlinestatus as onlinestatus,d.lastlogin as lastlogin,b.expectedcurrency,b.typesalary,c.submitdate,c.candidateid as candidateid, c.recruiterid as recruiterid , c.viewed,c.viewedtime,s.firstname as subuser,d.firstname as recsubuser,d.email as subuseremail FROM submitted_benchcandidates c,benchjobs a,benchcandidates b ,companyprofile e,members d ,members s  WHERE s.companyid ='$companyid' and s.memberid = a.memberid and a.jobid=c.jobid and b.id=c.candidateid and a.jobtitle like '%$jobtitle%' and ( b.resume like '%$candidate%' or c.candidateid='$candidate') and c.recruiterid= d.memberid and d.companyid=e.id  order by submitdate desc";

}else{
  $sql="SELECT a.jobtitle as jobtitle,a.jobid,d.email as recruiteremail,e.name as agency,e.registerid, b.fname as candidatename,b.contactnumber,b.cv as cv,b.resume,c.status as candidatestatus,c.prestatus,c.viewnotes,c.comment,b.jobtype,b.desiredsalary,d.onlinestatus as onlinestatus,d.lastlogin as lastlogin,b.expectedcurrency,b.typesalary,c.submitdate,c.candidateid as candidateid, c.recruiterid as recruiterid , c.viewed,c.employernotes,c.recruiternotes,c.viewedtime FROM submitted_benchcandidates c,benchjobs a,benchcandidates b ,companyprofile e,members d WHERE a.memberid='$mid' and a.jobid=c.jobid and b.id=c.candidateid and a.jobtitle like '%$jobtitle%' and ( b.resume like '%$candidate%' or c.candidateid='$candidate') and c.recruiterid= d.memberid and d.companyid=e.id  order by submitdate desc";
...
$agency=$_GET['agency'];
if($adminrights){
  $sql="SELECT a.jobtitle as jobtitle,a.jobid,d.email as recruiteremail,e.name as agency,e.registerid, b.fname as candidatename,b.contactnumber,b.cv as cv,b.resume,c.status as candidatestatus,c.prestatus,c.viewnotes,c.comment,b.desiredsalary,b.expectedcurrency,d.onlinestatus as onlinestatus,d.lastlogin as lastlogin,b.typesalary,c.submitdate,c.candidateid as candidateid, c.recruiterid as recruiterid,c.viewed,c.employernotes,c.recruiternotes,c.viewedtime,s.firstname as subuser,d.firstname as recsubuser,d.email as subuseremail FROM submitted_benchcandidates c,benchjobs a,benchcandidates b ,companyprofile e,members d ,members s  WHERE s.companyid ='$companyid' and s.memberid = a.memberid and a.jobid=c.jobid and b.id=c.candidateid and a.jobtitle like '%$jobtitle%'  and c.recruiterid= d.memberid and d.companyid=e.id and e.name ='$agency' and submitdate >= CURDATE()-INTERVAL 6 MONTH order by submitdate desc";

}else{
  $sql="SELECT a.jobtitle as jobtitle,a.jobid,d.email as recruiteremail,e.name as agency,e.registerid, b.fname as candidatename,b.contactnumber,b.cv as cv,b.resume,c.status as candidatestatus,d.onlinestatus as onlinestatus,d.lastlogin as lastlogin,c.prestatus,c.viewnotes,c.employernotes,c.recruiternotes,c.comment,b.jobtype,b.desiredsalary,b.expectedcurrency,b.typesalary,c.submitdate,c.candidateid as candidateid, c.recruiterid as recruiterid,c.viewed,c.viewedtime FROM submitted_benchcandidates c,benchjobs a,benchcandidates b ,companyprofile e,members d WHERE a.memberid='$mid' and a.jobid=c.jobid and b.id=c.candidateid and a.jobtitle like '%$jobtitle%'  and c.recruiterid= d.memberid and d.companyid=e.id and e.name ='$agency' and submitdate >= CURDATE()-INTERVAL 6 MONTH order by submitdate desc";
...
if($status=='CV Rejected')
	...
	$filter="c.status='$status'";
  if($adminrights){
    $sql="SELECT a.jobtitle as jobtitle,a.jobid,d.email as recruiteremail,e.name as agency,e.registerid, b.fname as candidatename,b.contactnumber,b.cv as cv,b.resume,c.status as candidatestatus,c.prestatus,c.viewnotes,c.comment,b.jobtype,b.desiredsalary,d.onlinestatus as onlinestatus,d.lastlogin as lastlogin,b.expectedcurrency,b.typesalary,c.submitdate,c.candidateid as candidateid, c.recruiterid as recruiterid,c.viewed,c.viewedtime,c.employernotes,c.recruiternotes,s.firstname as subuser,d.firstname as recsubuser,d.email as subuseremail FROM submitted_benchcandidates c,benchjobs a,benchcandidates b ,companyprofile e,members d ,members s  WHERE s.companyid ='$companyid' and s.memberid = a.memberid and a.jobid=c.jobid and b.id=c.candidateid and a.jobtitle like '%$jobtitle%'  and c.recruiterid= d.memberid and d.companyid=e.id  and $filter and submitdate >= CURDATE()-INTERVAL 6 MONTH order by submitdate desc";
...
if($status=='CV Rejected')
  ...
  if($adminrights){
    ...
    $sql="SELECT a.jobtitle as jobtitle,a.jobid,d.email as recruiteremail,e.name as agency,e.registerid, b.fname as candidatename,b.contactnumber,b.cv as cv,b.resume,c.status as candidatestatus,d.onlinestatus as onlinestatus,d.lastlogin as lastlogin,c.prestatus,c.viewnotes,c.comment,b.jobtype,b.desiredsalary,b.expectedcurrency,b.typesalary,c.submitdate,c.candidateid as candidateid, c.recruiterid as recruiterid , c.viewed,c.viewedtime,c.employernotes,c.recruiternotes FROM submitted_benchcandidates c,benchjobs a,benchcandidates b ,companyprofile e,members d WHERE a.memberid='$mid' and a.jobid=c.jobid and b.id=c.candidateid and ( b.resume like '%$candidate%' or c.candidateid='$candidate') and c.recruiterid= d.memberid and d.companyid=e.id and $filter and submitdate >= CURDATE()-INTERVAL 6 MONTH order by submitdate desc";
...
if($adminrights){
  ...
  $sql="SELECT a.jobtitle as jobtitle,a.jobid,d.email as recruiteremail,e.name as agency,e.registerid, b.fname as candidatename,b.contactnumber,b.cv as cv,b.resume,c.status as candidatestatus,c.prestatus,c.viewnotes,c.employernotes,c.recruiternotes,d.onlinestatus as onlinestatus,d.lastlogin as lastlogin,c.comment,b.jobtype,b.desiredsalary,b.expectedcurrency,b.typesalary,c.submitdate,c.candidateid as candidateid, c.recruiterid as recruiterid ,c.viewed,c.viewedtime FROM submitted_benchcandidates c,benchjobs a,benchcandidates b ,companyprofile e,members d  WHERE a.memberid='$mid' and a.jobid=c.jobid and b.id=c.candidateid and a.jobtitle like '%$jobtitle%' and c.recruiterid= d.memberid and d.companyid=e.id  and submitdate >= CURDATE()-INTERVAL 6 MONTH order by submitdate desc";
...
if($status=='CV Rejected')
  ...
  if($adminrights){
    $sql="SELECT a.jobtitle as jobtitle,a.jobid,d.email as recruiteremail,e.name as agency,e.registerid, b.fname as candidatename,b.contactnumber,b.cv as cv,b.resume,c.status as candidatestatus,d.onlinestatus as onlinestatus,d.lastlogin as lastlogin,c.prestatus,c.employernotes,c.recruiternotes,c.viewnotes,c.comment,b.jobtype,b.desiredsalary,b.expectedcurrency,b.typesalary,c.submitdate,c.candidateid as candidateid, c.recruiterid as recruiterid ,c.viewed,c.viewedtime,s.firstname as subuser,d.firstname as recsubuser,d.email as subuseremail FROM submitted_benchcandidates c,benchjobs a,benchcandidates b ,companyprofile e,members d, members s WHERE s.companyid ='$companyid' and s.memberid = a.memberid and a.jobid=c.jobid and b.id=c.candidateid  and ( b.resume like '%$candidate%' or c.candidateid='$candidate') and c.recruiterid= d.memberid and d.companyid=e.id and e.name ='$agency' and  $filter and submitdate >= CURDATE()-INTERVAL 6 MONTH order by submitdate desc";
...
if($status=='CV Rejected')
	...
	$filter=" c.status='$status'";
  if($adminrights){
    $sql="SELECT a.jobtitle as jobtitle,a.jobid,d.email as recruiteremail,e.name as agency,e.registerid, b.fname as candidatename,b.contactnumber,b.cv as cv,b.resume,c.status as candidatestatus,c.prestatus,c.employernotes,c.recruiternotes,c.viewnotes,c.comment,d.onlinestatus as onlinestatus,d.lastlogin as lastlogin,b.jobtype,b.desiredsalary,b.expectedcurrency,b.typesalary,c.submitdate,c.candidateid as candidateid, c.recruiterid as recruiterid , c.viewed,c.viewedtime,s.firstname as subuser,d.firstname as recsubuser,d.email as subuseremail FROM submitted_benchcandidates c,benchjobs a,benchcandidates b ,companyprofile e,members d,members s WHERE s.companyid ='$companyid' and s.memberid = a.memberid and a.jobid=c.jobid and b.id=c.candidateid and a.jobtitle like '%$jobtitle%'  and c.recruiterid= d.memberid and d.companyid=e.id and e.name ='$agency' and  $filter and submitdate >= CURDATE()-INTERVAL 6 MONTH order by submitdate desc";
  }else{
    $sql="SELECT a.jobtitle as jobtitle,a.jobid,d.email as recruiteremail,e.name as agency,e.registerid, b.fname as candidatename,b.contactnumber,b.cv as cv,b.resume,c.status as candidatestatus,d.onlinestatus as onlinestatus,d.lastlogin as lastlogin,c.prestatus,c.employernotes,c.recruiternotes,c.viewnotes,c.comment,b.jobtype,b.desiredsalary,b.expectedcurrency,b.typesalary,c.submitdate,c.candidateid as candidateid, c.recruiterid as recruiterid , c.viewed,c.viewedtime FROM submitted_benchcandidates c,benchjobs a,benchcandidates b ,companyprofile e,members d WHERE a.memberid='$mid' and a.jobid=c.jobid and b.id=c.candidateid and a.jobtitle like '%$jobtitle%'  and c.recruiterid= d.memberid and d.companyid=e.id and e.name ='$agency' and  $filter and submitdate >= CURDATE()-INTERVAL 6 MONTH order by submitdate desc";
...
<div class="account-navi-content">
  ...
  <div class="row">
   ...
   <div class="panel-body">
        <div class="text-left">
 Go to -> <a href="benchall" id="one"><strong>ALL CANDIDATES (<?php echo $total; ?>)</strong></a> -> <a href="benchawaiting-feedback" id="two">AWAITING FEEDBACK (<?php echo $submittedcount; ?>)</a> -> <a href="benchshortlist" id="three">SHORTLISTED (<?php echo $submittedcount1; ?>)</a> -> <a href="benchoffer" id="four">OFFERED (<?php echo $submittedcount2; ?>)</a> -> <a href="benchrejected" id="five">REJECTED (<?php echo $submittedcount3; ?>)</a> -> || <a href="benchjobs" id="six">JOBS ACTIVE</a> || <a href="benchfilled" id="seven">FILLED (<?php echo $submittedcount4; ?>)</a> || <a class="btn btn-primary" href="all-download<?php if($param)
 echo'?'.$param; ?>">
...
<h3>Total Candidates <b><?php echo $jobtitle; ?></b> - <b><?php echo $total; ?></b></h3>
      ...
      "> Viewed on '.$row['viewedtime'].' </small>';
        ...
        $checkonlinestatus = $row['onlinestatus'];
$checklastlogin = $row['lastlogin'];
 ...
 <td><b><a class="title" href="#" data-toggle="modal" data-target="#candidate-modal" id='.$row['candidateid'].'>'.$row['candidatename']. '</a></b> '.$viewed.'<br>
    ';	
  	...
  	echo ' | '.$row['candidateid'].'</small> |  '.$row['jobtype'].' <br><small>Employer Notes: <b>'.$row['employernotes'].'</b></small> <br><small>Reason: <b>'.$row['comment'].'</b></small> <br><small>Recruiter Notes: <b>'.$row['recruiternotes'].'</b></small></td>
    
<td><a href="/employer/benchall?jobtitle='.$row['jobtitle'].'&candidate=&agency=&status=&submit=Search" >'.$row['jobtitle'].'<br><small>'.$row['jobid'].'</small></td>
  ...
  if($row['candidatestatus']=="Awaiting Feedback" || $row['candidatestatus']=="Shortlisted"){
    ...
    <div class="btn-group">
      ...
      <ul class="dropdown-menu dropdown-menu--right" role="menu">';
		...
		{
		echo '
		    <li><a title="Select Candidate" href="benchselect-candidate?candidateid='.$row['candidateid'].'"><b>Shortlist</b></a></li>
  ...
  if($adminrights){
    echo '<td><b>'.$row['subuser'].'</b></td>';
	 ...
	 echo '<td><a class="title" href="#" data-toggle="modal" data-target="#recruiter-modal" id='.$row['registerid'].'>'.$row['agency'].'</a> <br>'; 
    	if(($checkonlinestatus!='0')){echo' <span class="label label-primary">Recruiter Online Now</span>'; } else {echo date("j F, Y",strtotime($row['lastlogin']));}
