...
$content="You have no engaged jobs matching this criteria in last 6 months. Click here to Search for New Roles <a href='searchbenchjobs'><b>Search New Bench Roles</b></a>";
...
<div class="account-navi-content">
  ...
  <div class="row">
    ...
    <h3 class="panel-title"> Engaged Jobs | Client name displayed next to Job title | Start submitting candidates by using the action button and earn 2X faster </h3>
  </header>
     ...
     <form method="get" action="job">
       ...
       <div class="row">
        ...
        <input name="employer" class="form-control ui-autocomplete-input" id="employer" placeholder="Search by Client Name (Start typing Client name and you will receive the list in the drop down)" autocomplete="off" type="text">
        </div>
        <div class="col-sm-4">
        <div class="form-group">
...
$checkrpo=$row['rpo'];
    ...
    if(!(in_array($row['requeststatus'],array("Requested","Rejected"))))
  	echo '<a class="title engage-trigger" href="#" data-toggle="modal" data-target="#employer-modal" id='.$row['empid'].' ><b>'.$row['employer'].'</b></a> <br>';	
  	if(($checkonlinestatus!='0')){echo' <span class="label label-primary">Employer Online Now</span>'; } else {echo date("j F, Y",strtotime($row['lastlogin']));}
	echo '
  ...
  <td><b><a href="/employer/bench_all?jobtitle='.urlencode($row['jobtitle']).'&candidate=&employer=&status=&submit=Search" >'.subcount($link,$row['jobid']).'</b></td>
