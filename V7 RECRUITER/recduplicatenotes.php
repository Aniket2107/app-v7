...
elseif(isset($_POST['submit'])){
	...
	$recduplicatenotes=htmlspecialchars($_POST['reason'],ENT_QUOTES);
	
	$replacewith = "hiddentext";
    $recduplicatenotes = preg_replace("/[\w-]+@([\w-]+\.)+[\w-]+/", $replacewith, $recduplicatenotes);
    $recduplicatenotes = preg_replace("/^(\(?\d{3}\)?)?[- .]?(\d{3})[- .]?(\d{4})$/", $replacewith, $recduplicatenotes);
    $recduplicatenotes = preg_replace("/^(\(?\d{4}\)?)?[- .]?(\d{3})[- .]?(\d{4})$/", $replacewith, $recduplicatenotes);
	...
	mysqli_query($link,"Update jobs INNER JOIN submitted_candidates ON submitted_candidates.jobid = jobs.jobid set submitted_candidates.recduplicatenotes='$recduplicatenotes',submitted_candidates.recduplicatenotesdate=now() WHERE submitted_candidates.candidateid='$candidateid' and jobs.memberid='$jmid' and submitted_candidates.jobid='$jid'");
	...
	$namerow1=mysqli_fetch_assoc($namesql1);
		...
		$subject = "Recruiting Hub - (".$namerow1['fname'].") Recruiter Duplicate Response Notes Added for ".$jobtitle." by ".$fetchempname['name']."";
		$message = '<html>
		<body bgcolor="#FFFFFF">
			<div>
        ...
        <table border="0" cellpadding="0" cellspacing="0" width="100%">
          <tbody>
            ...
            color: #a02121;">  PLEASE DO NOT REPLY TO THIS EMAIL AS EMAILS ARE NOT MONITORED FOR THIS EMAIL ID. LOGIN TO YOUR ACCOUNT TO RESPOND AS APPROPRIATE </p>   
					  ...
					  <p>Recruiter Duplicate Response Notes added by <strong>'.$fetchempname['name'].'</strong> for <strong>'.$namerow1['fname'].'</strong> submitted for <strong>'.$jobtitle.'</strong> </p>
					 <p> NOTES ADDED:<p style = "font-size: 14px;
            color: #a02121;"> <strong>'.$recduplicatenotes.'</strong></p>
...
'subject'=>$subject,'message'=>$message,'cc'=>$r['email'],'send'=>true];
	AsyncMail($params);
	 ...
	 echo "<script>
alert('Recruiter Duplicate Response Notes Successfully Added. An email notification has now been sent to the Employer');
...
?>
<?php  foreach($nameres as $namevalue) { echo '<h4 class="modal-title" id="duplicate-modal-label">'.$namevalue['fname'].'</h4>'; } ?>
<div id="feedback_name"><strong>Add Recruiter Duplicate Response Notes - (An Automated Email Notification will be sent to the Employer) <br> Please note that a copy of the email will also be sent to the Candidate ccing you.</strong> </div>
                   <?php if(!$errormsg){ 
			  			if($content)
							echo $content;
						else{ ?>
                            <form action="recduplicatenotes" method="post">
